...
export async function POST(request: Request) {
  try {
    ...
    try {
      await db().prepare(
        "INSERT INTO records (id, kind, rate, record_date, person_name, amount, unit, purpose, receipt_key, receipt_type, receipt_name, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"
      ).bind(id, kind, rate, recordDate, personName, amount, unit, purpose, receiptKey, receipt.type, receipt.name.slice(0, 200), createdAt).run();
    } catch (error) {
      await bucket().delete(receiptKey).catch(() => undefined);
      throw error;
    }
    return Response.json({ record: { id, kind, rate, record_date: recordDate, person_name: personName, amount, unit, purpose, receipt_type: receipt.type, receipt_name: receipt.name.slice(0, 200), created_at: createdAt, confirmed_by: null, confirmed_at: null, has_signature: false } }, { status: 201 });
  } catch (error) {
